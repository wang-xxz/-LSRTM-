...
#endif
#define NN 2
static int nb, nz, nx, nt, ns,nss,nss, ng, nzpad, nxpad;
static int *sxz, *gxz;
static float dz, dx, dt, fm, c0, c11, c12, c21, c22;
static float *wlt, *bndr, *dobs,*facc;
...
#pragma omp parallel for default(none)			\
	...
	for (iz=2; iz < nzpad-2; iz++) 
	{
	    ...
	    p0[ix][iz]=2*p1[ix][iz]-p0[ix][iz]+vv[ix][iz]*(c0*p1[ix][iz]+
...
#ifdef _OPENMP
#pragma omp parallel for default(none)			\
    private(ix,iz,px,pz,iN)					\
    shared(p1,p0,vv,nzpad,nxpad,c0,c11,c12,c21,c22,facc,dx,dz,dt,nb)  
#endif	
    for (ix=0; ix < nxpad; ix++) 
	for (iz=0; iz < nzpad; iz++) 
	{
		...
		{
			/////////x方向/////////
			if (((ix + iN) >= nxpad) && ((ix - iN < 0))) //x方向左右边界均超界
				px= 0;
			if (((ix + iN) < nxpad) && ((ix - iN) >= 0)) //x方向均在边界内
				px = px + facc[iN] *(p1[ix-iN][iz]+p1[ix+iN][iz]);
			if (((ix + iN) >= nxpad) && ((ix - iN) > 0)) //x方向右边界超界
				px = px + facc[iN] * p1[ix-iN][iz];
			if (((ix + iN) < nxpad) && ((ix - iN) < 0))  //x方向左边界超界
				px = px + facc[iN] * p1[ix+iN][iz];
			///////////z方向//////////		
			if (((iz + iN) >= nzpad) && ((iz - iN < 0))) //z方向左右边界均超界
				pz = 0;
			if (((iz + iN) < nzpad) && ((iz - iN >= 0))) //z方向均在边界内
				pz = pz + facc[iN] * (p1[ix][iz+iN] + p1[ix][iz-iN]);
			if (((iz + iN) >= nzpad) && ((iz - iN) >= 0))//z方向右边界超界
				pz = pz + facc[iN] * p1[ix][iz-iN];
			if (((iz + iN) < nzpad) && ((iz - iN) < 0))  //z方向左边界超界
				pz = pz + facc[iN] * p1[ix][iz+iN];
		}		
		px = px + facc[0] * p1[ix][iz];
		pz = pz + facc[0] * p1[ix][iz];

		
		
	       

		p0[ix][iz] = 2 * p1[ix][iz]-p0[ix][iz] + (vv[ix][iz] /(dx*dx))*px + (vv[ix][iz]/(dz*dz))*pz;*/
...
void add_source(int *sxz, float **p, int n, float *source, bool add)
/*< add source term >*/
{
    int is, sx, sz;
    if(add){
...
{
    bool csdgather,mute;
    int iz, ix, ib,is, it, jsx, jsz, jgx, jgz, sxbeg, szbeg, gxbeg, gzbeg, distx, distz, ntd,gx,gz,ig,nump;
    float *trans, tmp, amp, vmute,*dat;
    clock_t start, end;
    sf_file vinit, shots;
    ...
    /* total shots */
    if (!sf_getint("nss",&nss))   sf_error("no nss");	
    /* total shots */
    ...
    sf_putint(shots,"n3",nss);
    sf_putint(shots,"ns",ns);
    sf_putfloat(shots,"d1",dt);
    sf_putfloat(shots,"d2",jgx*dx);
    sf_putfloat(shots,"d3",1);
    ...
    dat=(float*)malloc(ng*nt*nss*sizeof(float));
    ...
    sf_warning("dx=%f,dz=%f,dt=%f,vv[0][0]=%f", dx,dz,dt,v0[0][0]);	
    for(ix=0;ix<nxpad;ix++){
	for(iz=0;iz<nzpad;iz++){
	    tmp=vv[ix][iz]*dt;
	...
	for(it=0; it<nt; it++)
	{
	    add_source(&sxz[is], p1, nump, &wlt[it], true);
	    step_forward(p0, p1);
	    ptr=p0; p0=p1; p1=ptr;
	    apply_sponge(p1);
	    apply_sponge(p0);
	    ...
	    for(ig=0;ig<ng; ig++){//record data
		    gx=gxz[ig]/nz;
		    gz=gxz[ig]%nz;
		    dat[it+nt*(ig+is*ng)]+=p0[gx+nb][gz];
		}
    ...
    sf_floatwrite(dat, ng*nt*nss, shots);
