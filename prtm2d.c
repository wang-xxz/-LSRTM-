...
static int *sxz, *gxz;
static float c0, c11, c21, c12, c22, dt,dx,dz;
static float *wlt,*wlt2, *bndr,*rwbndr, *mod, *dat,*facc;
static float **sp0, **sp1,**sp2, **gp0, **gp1, **vv, **ptr=NULL;
...
#pragma omp parallel for			\
	...
	for(ix=0; ix<nx; ix++)
	    for(iz=0; iz<2; iz++)
	    {
		p[ix+nb][iz]=spo[iz+4*ix];
		p[ix+nb][iz-2+nz]=spo[iz+2+4*ix];
	    }
#ifdef _OPENMP
#pragma omp parallel for			\
    private(ix,iz)				\
	...
	for(iz=0; iz<nz; iz++)
	    for(ix=0; ix<2; ix++)
	    {
		p[ix+nb][iz]=spo[4*nx+iz+nz*ix];
		p[ix-2+nx+nb][iz]=spo[4*nx+iz+nz*(ix+2)];
	    }
    }else{
#ifdef _OPENMP
#pragma omp parallel for			\
	...
	for(ix=0; ix<nx; ix++)
	    for(iz=0; iz<2; iz++)
	    {
		spo[iz+4*ix]=p[ix+nb][iz];
		spo[iz+2+4*ix]=p[ix+nb][iz-2+nz];
	    }
#ifdef _OPENMP
#pragma omp parallel for			\
    private(ix,iz)				\
	...
	for(iz=0; iz<nz; iz++)
	    for(ix=0; ix<2; ix++)
	    {
		spo[4*nx+iz+nz*ix]=p[ix+nb][iz];
		spo[4*nx+iz+nz*(ix+2)]=p[ix-2+nx+nb][iz];
	    }
    }
}
...
#ifdef _OPENMP
#pragma omp parallel for default(none)			\
    private(i2,i1,px,pz,iN)					\
    shared(nzpad,nxpad,u1,vv,u0,c0,c11,c12,c21,c22,facc,dx,dz)
#endif
	for (i2=2; i2<nxpad-2; i2++) 
	    for (i1=2; i1<nzpad-2; i1++) 
	    {
		...
		{
			/////////x方向/////////
			if (((i2 + iN) >= nxpad) && ((i2 - iN < 0))) //x方向左右边界均超界
				px= 0;
			if (((i2 + iN) < nxpad) && ((i2 - iN) >= 0)) //x方向均在边界内
				px = px + facc[iN] *(u1[i2-iN][i1]+u1[i2+iN][i1]);
			if (((i2 + iN) >= nxpad) && ((i2 - iN) > 0)) //x方向右边界超界
				px = px + facc[iN] * u1[i2-iN][i1];
			if (((i2 + iN) < nxpad) && ((i2 - iN) < 0))  //x方向左边界超界
				px = px + facc[iN] * u1[i2+iN][i1];
			///////////z方向//////////		
			if (((i1 + iN) >= nzpad) && ((i1 - iN < 0))) //z方向左右边界均超界
				pz = 0;
			if (((i1 + iN) < nzpad) && ((i1 - iN >= 0))) //z方向均在边界内
				pz = pz + facc[iN] * (u1[i2][i1+iN] + u1[i2][i1-iN]);
			if (((i1 + iN) >= nzpad) && ((i1 - iN) >= 0))//z方向右边界超界
				pz = pz + facc[iN] * u1[i2][i1-iN];
			if (((i1 + iN) < nzpad) && ((i1 - iN) < 0))  //z方向左边界超界
				pz = pz + facc[iN] * u1[i2][i1+iN];
		}		
		px = px + facc[0] * u1[i2][i1];
		pz = pz + facc[0] * u1[i2][i1];

		u0[i2][i1] = 2 * u1[i2][i1]-u0[i2][i1] + (vv[i2][i1] /(dx*dx))*px + (vv[i2][i1]/(dz*dz))*pz;*/
...
{  
    ...
    for(it=0; it<nt; it++){
	t=SF_PI*fm*(it*dt_-1/fm);t=t*t;
	t1=SF_PI*fm*((it+1)*dt_-1/fm);t1=t1*t1;
	t2=SF_PI*fm*((it-1)*dt_-1/fm);t2=t2*t2;
	wlt[it]=(amp*(1.0-2.0*t1)*expf(-t1)-2*amp*(1.0-2.0*t)*expf(-t)+amp*(1.0-2.0*t2)*expf(-t2));//二阶
	//wlt[it]=-(amp*(1.0-2.0*t1)*expf(-t1)-amp*(1.0-2.0*t2)*expf(-t2))/(2*dt);//一阶
        //wlt[it]=amp*(1.0-2.0*t)*expf(-t);
        wlt2[it]=amp*(1.0-2.0*t)*expf(-t);
    }
...
void illumination(int nm, int nd, float *illum, float *dat)
/*< prtm2d linear operator >*/
{
	...
	float *illums = sf_floatalloc(nm);
	float *illumr = sf_floatalloc(nm);
	memset(illums, 0, nz * nx * sizeof(float));
	memset(illumr, 0, nz * nx * sizeof(float));
    for(is=0; is<ns; is++) {/* it may be parallized using MPI */
	/* initialize is-th source wavefield Ps[] */
	memset(sp0[0], 0, nzpad*nxpad*sizeof(float));
	memset(sp1[0], 0, nzpad*nxpad*sizeof(float));
	memset(sp2[0], 0, nzpad*nxpad*sizeof(float));
	memset(gp0[0], 0, nzpad*nxpad*sizeof(float));
	memset(gp1[0], 0, nzpad*nxpad*sizeof(float));
		...
		add_source(&sxz[is], sp1, 1, &wlt2[it], false);
		step_forward(sp0, sp1, vv, false);
		ptr=sp0; sp0=sp1; sp1=ptr;
		apply_sponge(sp0);
		apply_sponge(sp1);
		...
		for(ig=0;ig<ng; ig++){
		    gx=gxz[ig]/nz;
		    gz=gxz[ig]%nz;
		    gp1[gx+nb][gz]+=dat[it+ig*nt+is*nt*ng];
		}
		...
		for(i2=0; i2<nx; i2++)
			...
			illums[i1+nz*i2]+=sp1[i2+nb][i1]*sp1[i2+nb][i1];
			illumr[i1+nz*i2]+=gp1[i2+nb][i1]*gp1[i2+nb][i1];
			}
		...
		for(i2=0; i2<nx; i2++)
		    for(i1=0; i1<nz; i1++)
			illum[i1+nz*i2]=illumr[i1+nz*i2]*illums[i1+nz*i2];
...
/*< prtm2d linear operator >*/
{
	...
	if(adj){/* migration: mm=Lt dd */
		...
		for(i2=0; i2<nx; i2++)
		    for(i1=0; i1<nz; i1++){
			mod[i1+nz*i2]+=sp1[i2+nb][i1]*gp1[i2+nb][i1];
			//mod[i1+nz*i2]+=(sp0[i2+nb][i1+nb]-sp1[i2+nb][i1+nb])*gp1[i2+nb][i1+nb];
			//mod[i1+nz*i2]+=(sp2[i2+nb][i1+nb]-2*sp0[i2+nb][i1+nb]+sp1[i2+nb][i1+nb])*gp1[i2+nb][i1+nb];
			}
		...
		for(i2=0; i2<nx; i2++)
		    for(i1=0; i1<nz; i1++)
			//gp1[i2+nb][i1+nb]+=(sp1[i2+nb][i1+nb]-sp0[i2+nb][i1+nb])*mod[i1+nz*i2];
			//gp1[i2+nb][i1+nb]+=(sp2[i2+nb][i1+nb]-2*sp0[i2+nb][i1+nb]+sp1[i2+nb][i1+nb])*mod[i1+nz*i2];
			gp1[i2+nb][i1]+=sp1[i2+nb][i1]*mod[i1+nz*i2];
		step_forward(gp0, gp1, vv, false);	
		ptr=gp0; gp0=gp1; gp1=ptr;
		apply_sponge(gp0); 
		apply_sponge(gp1); 
		for(ig=0;ig<ng; ig++){//record data
		    gx=gxz[ig]/nz;
		    gz=gxz[ig]%nz;
		    dat[it+ig*nt+is*nt*ng]+=gp1[gx+nb][gz];
		}
