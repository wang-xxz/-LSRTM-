...
#ifdef _OPENMP
#pragma omp parallel for default(none)			\
    private(ix,iz,px,pz,iN,alfx,alfz)					\
	...
	for (iz=0; iz < nzpad; iz++) 
	{
		...
		if ((ix >= nb && ix < nxpad-nb) && (iz >= nzpad-nb && iz < nzpad))
		{
			alfx= 0;
			alfz= B * (1 - cos(SF_PI*(iz - (nzpad-nb)) / (2 * nb)));
		}
	///////////区域②，alfx!=0,alfz=0/////////////////////
		else if ((ix >= 0 && ix < nb) && (iz >= 0 && iz < nzpad-nb))
		{
			alfx = B * (1 - cos(SF_PI*(nb - ix) / (2 * nb)));
			alfz = 0;
		}
		else if ((ix >= nxpad-nb && ix < nxpad) && (iz >= 0 && iz < nzpad-nb))
		{
			alfx = B * (1 - cos(SF_PI*(ix - (nxpad-nb)) / (2 * nb)));
			alfz = 0;
		}
	/////////////区域③，alfx!=0,alfz!=0/////////////////
		else if ((ix >= 0 && ix < nb) && (iz >= nzpad-nb && iz < nzpad))
		{
			alfx = B * (1 - cos(SF_PI*(nb - ix) / (2 * nb)));
			alfz = B * (1 - cos(SF_PI*(iz - (nzpad-nb)) / (2 * nb)));
		}
		else if ((ix >= (nxpad-nb) && ix < nxpad) && (iz >= nzpad-nb && iz < nzpad))
		{
			alfx = B * (1 - cos(SF_PI*(ix - (nxpad-nb)) / (2 * nb)));
			alfz = B * (1 - cos(SF_PI*(iz - (nzpad-nb)) / (2 * nb)));
		}
	...
	/////////////////////////////////////////////////////////////////////////
		...
		p0[ix][iz]=((2-(alfx+ alfz)*dt*dt)/(1 + (alfx+ alfz) * dt))*p1[ix][iz]-((1 - (alfx+ alfz) * dt)/(1 + (alfx+ alfz) * dt))*p0[ix][iz]+(vv[ix][iz])*px/((1 + (alfx+ alfz) * dt)*dx*dx)+(vv[ix][iz])*pz/((1 + (alfx+ alfz) * dt)*dz*dz);
...
int main(int argc, char* argv[])
{
	...
	//bndr[ib]=80 * (1 - cos(SF_PI*(nb-ib) / (2 * nb)));
    }
    for(ix=0;ix<nxpad;ix++){
	for(iz=0;iz<nzpad;iz++){
	    tmp=vv[ix][iz]*dt;
